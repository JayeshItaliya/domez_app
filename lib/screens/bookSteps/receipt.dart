...
class _ReceiptState extends State<Receipt> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<CommonController>(builder: (controller) {
      return WillPopScope(
        onWillPop: () async {
          return isFullPaymentAPICalling || isSplitPaymentAPICalling
              ? false
              : true;
        },
        child: SafeArea(
          ...
          child: Scaffold(
            ...
            extendBody: false,
            body: AbsorbPointer(
              absorbing: isFullPaymentAPICalling || isSplitPaymentAPICalling,
              child: ListView(
                shrinkWrap: true,
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      NunitoText(
                                                                          ...
                                                                          color:
                                                                              AppColor.grey),
                                                                      NunitoText(
                                                                          text: cx.read(Keys.startTime).toString().substring(0, 8) +
                                                                              " To " +
                                                                              cx.read(Keys.endTime).toString().substring(11,
                                                                                  19),
                                                                          fontSize: cx.height > 800
                                                                              ? 17
                                                                              : 15,
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      children: [
                                                                        NunitoText(
                                                                            textAlign:
                                                                                TextAlign.end,
                                                                            text: "\$" + cx.read(Keys.price).toStringAsFixed(2),
                                                                            fontSize: cx.responsive(25, 20, 17),
                                                                        ...
                                                                        NunitoText(
                                                                            textAlign:
                                                                                TextAlign.end,
                                                                            text: "+ \$" + cx.read(Keys.totalHST).toStringAsFixed(2),
                                                                            fontSize: cx.responsive(25, 20, 17),
                                                                            fontWeight: FontWeight.w700,
                                                                            color: Color(0xFF757575)),
                                                                      ],
              ...
              child: Container(
                ...
                child: Form(
                  ...
                  child: Container(
                    width: cx.width,
                    height:
                        cx.read("islogin") ? cx.height / 8 : cx.height / 3.4,
                    color: AppColor.darkGreen,
                    child: Padding(
                                        ...
                                        if (!isDataProcesssing) {
                                          if (mounted) {
                                            if (cx.read("islogin")) {
                                              ...
                                              print(startBookingTime.subtract(
                                                  Duration(hours: 4)));
                                              print(DateTime.now());
                                              print(startBookingTime
                    ...
                    child: InkWell(
                      onTap: () async {
                        var result = await Get.to(
                            SignUp(curIndex: 0, noOfPopTime: 99),
      ...
      await Stripe.instance.presentPaymentSheet().then((value) async {
        ...
        TaskProvider.paymentSuccessfulAPI(
          ...
          paid_amount: cx.isSplitAmount.value
              ? cx.read(Keys.splitPaidAmount).toStringAsFixed(2)
              : cx.read(Keys.total).toStringAsFixed(2),
          payment_method: "1",
          ...
          start_time: cx.read(Keys.startTime).substring(0, 8),
          team_name: "",
          total_amount: cx.read(Keys.total).toStringAsFixed(2),
          ...
          due_amount: cx.isSplitAmount.value
              ? cx.read(Keys.splitRemainingAmount).toStringAsFixed(2)
              : "0.0",
          hst: cx.read(Keys.totalHST).toStringAsFixed(2),
          service_fee: cx.read(Keys.serviceFee).toStringAsFixed(2),
          ...
          if (cx.isSplitAmount.value) {
            await _createDynamicLink(
                    true, "/domeBooking", value?.bookingId.toString() ?? "")
                .then((data) {
        ...
        print('Error in display payment sheet is:--->$error $stackTrace');
      });
    } on StripeException catch (e) {
      setState(() {
        isDataProcesssing = false;
      });
      print('Error in stripe exception is:---> $e');
      showDialog(
          context: context,
  ...
  _onAlertWithCustomContentPressed(context) {
    showDialog(
      ...
      builder: (BuildContext context) => WillPopScope(
          ...
          child: AbsorbPointer(absorbing: true, child: reserveSuccessful())),
    );
    Timer(du, () {
      if (!_isCreatingLink) {
        Get.back();
